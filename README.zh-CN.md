...
让 Pi 的 `/tree` 历史导航具备类似 Claude Code 那种让人敢放心改代码的工作区回退安全感。
...
它不是单纯给 `pi` 增加一个撤销快捷键，而是要让聊天历史和本地工作区文件状态一起回到过去、回到未来、或切换到任意历史分支节点。
...
- `/tree` 是真正的时间机器
- 撤销就是沿着 `/tree` 跳回某一轮的用户 prompt，重做就是再向前跳回刚才离开的位置
- `/checkpoint` 用于在导航前保护手动修改
...
- 一次 `/tree` 跳转就能撤销一整轮 Agent 的结果，而不只是恢复一部分文件
...
3. 当用户通过 `/tree` 切换历史节点时，工作区必须同步恢复。
4. 撤销某一轮时恢复的应该是“这一轮开始前的真实状态”，而不是简单回到上一轮 Agent 完成后的状态。
...
8. 多个 session 之间的快照和内部状态应该相互隔离，不能串台。
...
- 基于 `/tree` 的工作区恢复
  - 切换历史节点时自动恢复对应的工作区状态
  - 支持在不同历史分支之间来回切换
  - 跳到某一轮的 prompt 就是撤销该轮，再向前跳回离开的地方就是重做
...
- Session isolation
  - 每个 session 使用独立的 shadow git 和内部状态
  - 避免新会话导航时串到旧会话历史
